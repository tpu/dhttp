...
exports.j2h = {
 ...
 html    : '',
 file    : '', 
 init    : function(src, gObj){ 
  this.file = fs.readFileSync(src); 
  if(gObj){ this.g_o = gObj;  } 
 },
 run     : function(onErr,callback){
           events.once( 'success', callback );
  try{
   this.html = this.file.toString();
    while(true){
     if(!this.codeExp.test(this.html)){ break; }
      this.html = this.html.replace(this.codeExp, evalWithObj(this.codeExp.exec(this.html)[2], this.g_o, onErr));
    }
